...
H=0.225; %distancia CG a Roll Axis
Kf= 18370.86641/57.2958; %roll stiffnes front maxima ajustabilidad +150 de lo que de el muelle, en nm/deg
...
deltadg=-120:20:120; %Steering wheel angles en graus
...
for i = 1:length(deltadg);
     [intwheel_deg, extwheel_deg] = ackermann_function(deltadg(i));
    intwheel_rad= deg2rad(intwheel_deg); 
    extwheel_rad= deg2rad(extwheel_deg);
    for j = 1:length(VS);
        Vy(j)=(V/3.6).*VS(j); 
        w(i,j)=0;
        Ay(i,j)=0;
        for k=1:10     
            ...
            alphaFL(i,j)=((tan(VS(j))*(V/3.6))+(w(i,j)*a))./((V/3.6)-w(i,j)*Tf/2)-extwheel_rad; %slip angle front left en rad
            ...
            alphaFR(i,j)=((tan(VS(j))*(V/3.6))+(w(i,j)*a))./((V/3.6)+w(i,j)*Tf/2)-intwheel_rad; %slip angle front right en rad
            ...
            Ay(i,j)=(FyFL(i,j)*(cos(extwheel_rad))+FyFR(i,j)*(cos(intwheel_rad))+(FyRR(i,j)+FyRL(i,j)))/(weight*g); %Acceleracio lateral o centripeta en g
            ...
            YawMoment(i,j)=(FyFL(i,j).*cos(extwheel_rad)+FyFR(i,j).*cos(intwheel_rad))*a-(FyRR(i,j)+FyRL(i,j))*b+MZ(alphaFL(i,j))+MZ(alphaFR(i,j))+MZ(alphaRL(i,j))+MZ(alphaRR(i,j));
