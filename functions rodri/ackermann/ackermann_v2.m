...
dynamic_toe_ack = @(rack_disp) - atand(wheelbase/(wheelbase/tand(phi_delta(-rack_disp)) + rear_track)) + phi_delta(-rack_disp);

pctge_ack = @(rack_disp) dynamic_toe(rack_disp)/dynamic_toe_ack(rack_disp)

figure(1)
fplot(phi_delta, [-50, 50])

figure(2)
fplot(dynamic_toe, [0, 50])

figure(3)
fplot(dynamic_toe_ack, [0, 50])

figure(4)
fplot(pctge_ack, [0, 50])
