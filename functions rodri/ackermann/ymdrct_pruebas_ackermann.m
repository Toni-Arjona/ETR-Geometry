...
%Imputs
Radi=10; %Radi de gir
deltadg = 0.1:5:60.1; % Steering angles en graus
%SI=deltadg'.*pi/180; %Steering angle en rad
R=sign(deg2rad(deltadg))*Radi;
Betadg=0.1:0.5:10.1; %Slip angle en graus
    ...
    for j = 1:length(VS)
        if Q==0
            ...
            for k=1:10     
                ...
                alphaFL(i,j)=((tan(VS(j))*(w(i,j)*R(i)))+(w(i,j)*car.a))./((w(i,j)*R(i))-w(i,j)*car.Tf/2)-extwheel_rad; %slip angle front right en rad
                ...
                alphaFR(i,j)=((tan(VS(j))*(w(i,j)*R(i)))+(w(i,j)*car.a))./((w(i,j)*R(i))+w(i,j)*car.Tf/2)-intwheel_rad; %slip angle front right en rad
                ...
                if Q==0
                    Ay(i,j)=(FyFL(i,j)*(cos(extwheel_rad))+FyFR(i,j)*(cos(intwheel_rad))+(FyRR(i,j)+FyRL(i,j)))/(car.weight*g); %Acceleracio lateral o centripeta en g
                end
                ...
                YawMoment(i,j)=(FyFL(i,j).*cos(extwheel_rad)+FyFR(i,j).*cos(intwheel_rad))*car.a-(FyRR(i,j)+FyRL(i,j))*car.b +MZ(alphaFL(i,j))+MZ(alphaFR(i,j))+MZ(alphaRL(i,j))+MZ(alphaRR(i,j));
...
zlabel('FYFL[N]')
legendCell_VS = cellstr(num2str(VS*180/pi, 'VS=%-d'));
legendCell_SI = cellstr(num2str(deltadg, 'deltadg=%-d'));
legend([legendCell_VS;legendCell_SI], 'NumColumns',2)
