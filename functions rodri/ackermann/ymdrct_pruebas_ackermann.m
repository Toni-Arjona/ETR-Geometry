...
for i = 1:length(deltadg);
    ...
    deg2rad(intwheel_deg); deg2rad(extwheel_deg)
    ...
    for j = 1:length(VS); 
        if Q==0
            ...
            for k=1:10     
                ...
                alphaFL(i,j)=((tan(VS(j))*(w(i,j)*R(i)))+(w(i,j)*car.a))./((w(i,j)*R(i))-w(i,j)*car.Tf/2)-extwheel_deg; %slip angle front right en rad
                ...
                alphaFR(i,j)=((tan(VS(j))*(w(i,j)*R(i)))+(w(i,j)*car.a))./((w(i,j)*R(i))+w(i,j)*car.Tf/2)-intwheel_deg; %slip angle front right en rad
                ...
                if Q==0
                    Ay(i,j)=(FyFL(i,j)*(cos(extwheel_deg))+FyFR(i,j)*(cos(intwheel_deg))+(FyRR(i,j)+FyRL(i,j)))/(car.weight*g); %Acceleracio lateral o centripeta en g
                end
                ...
                YawMoment(i,j)=(FyFL(i,j).*cos(extwheel_deg)+FyFR(i,j).*cos(intwheel_deg))*car.a-(FyRR(i,j)+FyRL(i,j))*car.b +MZ(alphaFL(i,j))+MZ(alphaFR(i,j))+MZ(alphaRL(i,j))+MZ(alphaRR(i,j));
...
legendCell_SI = cellstr(num2str(deltadg*180/pi, 'SI=%-d'));
