...
% Pre-cálculo de valor absoluto para filtrar
p = abs(radio);

% 1. Definir la recta (lo que queda fuera del rango 0 a 100)
% Nota: Ajusté los comparadores a >= y < para no perder valores exactos (ej. el 5)
idx_recta = p <= 0 | p >= 100;
recta = radio(idx_recta);

% Denominador para los porcentajes
total_curva = length(radio) - length(recta);

% 2. Extraer vectores usando indexación lógica (corrige tu error de índices)
...
surface([x_track; x_track], [y_track; y_track], [z; z], [abs(curvatura); abs(curvatura)], ...
        'facecol', 'no', ...
        'edgecol', 'interp', ...
