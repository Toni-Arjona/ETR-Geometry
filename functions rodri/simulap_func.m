function tiempo_total = simulap_func(gear_ratio)
    ...
    circuito_v2 = readtable("C:\Users\rodri\OneDrive\Escritorio\ETECH\GitHub\ETR-Geometry\functions rodri\datos\puntos_circuito_performance_spline.csv");
    circuito_v1 = readtable("C:\Users\rodri\OneDrive\Escritorio\ETECH\GitHub\ETR-Geometry\functions rodri\datos\puntos_circuito_magazine.csv");
    ...
    %PARÁMETROS SIMU
        v = 15; % velocidad inicial [m/s] (se ha cogido velocidad final de vuelta de una simu forward cualquiera)
    ...
    % DEFINICIÓN TRAYECTO CIRCUITO CON SPLINES
        ...
        d_total = [0; cumsum(sqrt(diff(circuito_v2.X).^2 + diff(circuito_v2.Y).^2))];
        d_interval = 0: d : max(d_total);
        
        
        % 2. Definir exprsión splines circuito x e y, y evalurar para intervalo simu
        x_spline = spline(d_total, circuito_v2.X); x_track = ppval(x_spline, d_interval);
        y_spline = spline(d_total, circuito_v2.Y); y_track = ppval(y_spline, d_interval);
